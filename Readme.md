<div align="center">
    ...
    <p>云盘自动转存系统，支持移动云盘，自动监控更新并转存文件, 支持机器人操作。</p>
</div>
...
- 任务设置：
  ...
  - 任务重试次数和间隔
  - 重试任务检查 Cron：控制失败任务重试扫描频率（默认每10分钟）
  - 云盘请求并发上限（139）：默认 `3`，可按网络和风控策略调整
  - 定时检查时间（支持 Cron 表达式）
  - 自动清空回收站（支持个人和家庭网盘）
  - 云盘文件夹不存在时自动创建：当任务的目标更新目录被删除时，自动重新创建该目录（当前默认支持移动云盘，仅会创建最后一级文件夹）

### 6. 工程化配置补充（R6）
- **CORS 白名单**：通过环境变量 `ALLOWED_ORIGINS` 配置，多个来源以逗号分隔  
  例如：`ALLOWED_ORIGINS=http://localhost:3001,https://your-domain.com`
- **会话 Secret**：系统首次启动会自动生成并持久化到 `data/config.json` 的 `system.sessionSecret`
- **优雅停机**：已支持 `SIGTERM/SIGINT`，会按顺序关闭 HTTP 服务、SSE、定时任务、机器人与数据库连接
- 媒体设置：
  - STRM 文件生成：自动为媒体文件生成对应的 STRM 文件
  - Emby 通知：支持自动刷新 Emby 媒体库
    - 路径替换规则说明：
      1. 示例场景：
         - 云盘中的文件路径：`/影视剧/电视剧/北上/Season 01/S01E01.mkv`
         - Emby 媒体库中的路径：`/cloud/移动云盘/电视剧`
         - 账号中的Emby路径替换配置：`/影视剧:/cloud/移动云盘`
      
      2. 替换执行逻辑：
         - 系统会优先尝试完整路径匹配：
           `/cloud/移动云盘/电视剧/北上/Season 01`
         - 如果完整路径不存在，会逐级向上查找：
           `/cloud/移动云盘/电视剧/北上`
           `/cloud/移动云盘/电视剧`
...
2. 本项目当前版本提供移动云盘（139）转存能力，所有接口均为各平台官方接口，本项目不对任何资源内容负责。
