...
| **1.4.3**   | **D4 第一批进展：主入口接入模块化路由注册（`registerRoutes`）并使 D1/D3 路由链路生效；`TaskService` 的 cloud139 处理改为委托 `cloud139TaskProcessor`，统一重试退避与可见性轮询主流程；补充账号弹窗 `flex` 打开一致性回归检查。** |
